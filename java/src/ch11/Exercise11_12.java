...
public class Exercise11_12 {
	...
	public static void main(String[] args) throws Exception {
		...
		TreeMap rank = new TreeMap(new Comparator() {
			// comparator 일회성
			// comparable 클래스를 만들어서 object 정렬기준
			public int compare(Object o1, Object o2) {
				...
				if (o1 instanceof Player && o2 instanceof Player) {
					...
					return p2.point - p1.point; // 점수 내림차순
				}
				return -1;
			}
		});
...
class SutdaDeck {
	...
	int getPoint(Player p) {
		...
		// 카드 두 장이 모두 광이면, jokbo에서 키를 "KK"로 해서 점수를 조회한다
		if (c1.isKwang && c2.isKwang) {
			result = (Integer) jokbo.get("KK");
		} else {
			// 두 카드의 숫자(num)로 jokbo에서 등급을 조회한다
			result = (Integer) jokbo.get("" + c1.num + c2.num);

			// 해당하는 등급이 없으면 (c1.num+c2.num)%10+1000 으로 점수 계산
			if (result == null) {
				result = (c1.num + c2.num) % 10 + 1000;
			}
		}
		// player의 점수(point)에 계산한 값 저장
		p.point = result;
